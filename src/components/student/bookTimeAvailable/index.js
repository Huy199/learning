...
const BookTimeAvailable = () => {
  ...
  useEffect(() => {
    ...
    let newBookingTimes = timeSlotCacheRef.current.get(bookingDate.toDateString());
  ...
  return (
    <div id='bookingTimeAvailable' className='k-mt-8'>
      <div className='k-flex k-display-flex k-mb-8'>
        <Calendar value={bookingDate} onChange={onDateChange} />
        <div className='k-ml-4 k-display-flex k-flex-col'>
          {bookingTimes.map((time) => {
            return (
              <button key={time} className='timeValue_btn k-button k-mb-4' onClick={(e) => setSelectedTimeSlot(time)}>
                {time}
              </button>
            );
          })}
      ...
      <div className='bookingWrap'>
        ...
        <button className='bookingBtn'>Xác nhận</button>
      </div>
    </div>
  );
};

export default BookTimeAvailable;
